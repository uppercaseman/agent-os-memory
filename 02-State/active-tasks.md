...
2. [ ] **B. `bin/aos-up.sh` launcher.** Write `bin/aos-up.sh`, `bin/aos-down.sh`, `bin/aos-status.sh`. Idempotent, PID-tracked, logs to `~/AOS/logs/`. hermes-verify: cold start <10s, clean shutdown, no orphan processes.
